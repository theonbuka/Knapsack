...
def test_navigation_to_assets(page: Page):
    page.goto(BASE_URL)
    page.wait_for_load_state("networkidle")
    page.locator("a[href='/assets']").click()
    page.wait_for_load_state("networkidle")
    expect(page.locator("h1")).to_contain_text("Assets")
...
def test_navigation_to_transactions(page: Page):
    ...
    page.wait_for_load_state("networkidle")
    expect(page.locator("h1")).to_contain_text("Harcama Geçmişi")
...
def test_theme_toggle(page: Page):
    ...
    page.get_by_label("Switch to light mode").click()
