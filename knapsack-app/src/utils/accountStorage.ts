...
export function getAuthStorageId(auth: Partial<AuthState> | null | undefined): string {
  ...
  if (auth.email?.trim()) {
    return `email:${cleanSegment(auth.email)}`;
  }

  if (auth.googleId?.trim()) {
    return `google:${cleanSegment(auth.googleId)}`;
  }

  const fullName = `${auth.name || ''}.${auth.surname || ''}`.trim();
  if (fullName) {
    return `name:${cleanSegment(fullName)}`;
  }
...
export function getScopedDataKey(baseKey: string, storageId?: string): string {
  ...
  if (!resolvedStorageId) return baseKey;
